...
void VtoI_Com(void)
{
	...
	if (ULTRASONIC_Read()<CRITICAL_DISTANCE)
	{
		Car_Stop();
	}

	if (ESP_Get()==LANE_ASSIST)
	{
		ESP_Post(OK);
		...
		else if (ULTRASONIC_Read()<CRITICAL_DISTANCE)
		{
			ESP_Post(STOP);
		}
	}
...
void RC_Control(void)
{
	...
	{
		MOTOR_Speed(M1,_5THSPEED);
		MOTOR_Speed(M2,_5THSPEED);
		MOTOR_Speed(M3,_5THSPEED);
		MOTOR_Speed(M4,_5THSPEED);
	}
...
{
	if (IR_SENSOR1()==WHITE&&IR_SENSOR2()==WHITE)

	{
		moveForward();
	}
	else if (IR_SENSOR1()==BLACK&&IR_SENSOR2()==WHITE)

	{
		MOVE_RIGHT();
	}
	else if (IR_SENSOR1()==WHITE&&IR_SENSOR2()==BLACK)
